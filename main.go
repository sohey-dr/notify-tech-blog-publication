...
func run() {
	articles := concurrentScraping(
		...
		scraper.NewScraper("メルカリ", "https://engineering.mercari.com/", "time", time.Now().Format("2006/01/02"), ".post-list__item > a", false).Scrape,
		...
		scraper.NewScraper("ラクスル", "https://tech.raksul.com/", ".c-posts__post__date", time.Now().Format("2006.01.02"), ".c-posts__post__info__title > a", false).Scrape,
		scraper.NewScraper("Yahoo!", "https://techblog.yahoo.co.jp/", ".date", time.Now().Format("2006.01.02"), ".panel-horizontal > a", true).Scrape,
		scraper.NewScraper("サイボウズ", "https://blog.cybozu.io/", "time", time.Now().Format("2006-01-02"), ".entry-title > a", true).Scrape,
