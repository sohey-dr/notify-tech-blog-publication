...
func run() {
	articles := concurrentScraping(
		...
		// TODO: Slackで読み込み後に見れるため見逃しているがタイトルも取得するようにする(CA, メルカリ, エルレカ)
		...
		scraper.NewScraper("Yahoo!", "https://techblog.yahoo.co.jp/", ".date", time.Now().Format("2006.01.02"), ".post-list__item > a", true).Scrape,
